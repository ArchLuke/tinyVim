    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
          "tinyVim editor -- version %s", KILO_VERSION);
...
{
	char *seq=calloc(sizeof(char), E.screencols);
	int stop=0;
	int nread=0;
	
	for(int i=0;i<E.screencols;i++)
	{
		while ((nread = read(STDIN_FILENO, &seq[i], 1)) != 1) {
    			if (nread == -1 && errno != EAGAIN) die("read");
		}
...
void editorHandleNavigation(int c)
{
	switch(c)
	{
...
void editorProcessKeypress() {
  ...
  {
  	case DEFAULT:
		editorProcessDefaultKeypress(c);
		break;
	case INSERT:
...
int main(int argc, char *argv[]) {
  ...
  editorSetStatusMessage(
    "\"%s\" %dL, %dB", E.filename, E.numrows, E.size);
